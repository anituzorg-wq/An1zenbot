...
VIP_CARD_NUMBER = "7777 0105 7309 7248"
VIP_CARD_NAME = "Paynet karta"
VIP_CARD_OWNER = "M.Abuvakhobov"
VIP_CONTACT = "@An1zen"
VIP_PLANS = {
    "1 oylik": 30,
    "3 oylik": 90,
...
def user_start_keyboard():
    ...
    return ReplyKeyboardMarkup(
        keyboard=[[KeyboardButton(text="/start"), KeyboardButton(text="/vip")]],
...
def vip_purchase_keyboard():
    ...
    for plan in VIP_PLANS:
        b.button(text=f"💎 {plan}", callback_data=f"vipbuy:{plan}")
    b.adjust(1)
    return b.as_markup()


def vip_admin_keyboard():
    b = InlineKeyboardBuilder()
    b.button(text="➕ VIP berish", callback_data="vip:give")
    b.button(text="➖ VIP olish", callback_data="vip:remove")
    b.button(text="📋 VIP foydalanuvchilar", callback_data="vip:list")
    b.adjust(1)
    return b.as_markup()
...
def grant_vip(user_id: int, duration: str, granted_by: int):
    if duration == "forever":
        expires_at = None
    else:
        days = VIP_PLANS.get(duration)
        if days is None:
            raise ValueError("Noto'g'ri VIP muddati")
        expires_at = (datetime.now(timezone.utc) + timedelta(days=days)).isoformat()
    db.execute(
        "INSERT INTO vip_users(user_id, expires_at, granted_by, granted_at) VALUES (?,?,?,?) "
        "ON CONFLICT(user_id) DO UPDATE SET expires_at=excluded.expires_at, granted_by=excluded.granted_by, granted_at=excluded.granted_at",
        (user_id, expires_at, granted_by, now())
    )
    db.commit()


def remove_vip(user_id: int):
    db.execute("DELETE FROM vip_users WHERE user_id=?", (user_id,))
    db.commit()


async def get_vip_status(user_id: int):
    clear_expired_vips()
    return db.execute("SELECT * FROM vip_users WHERE user_id=?", (user_id,)).fetchone()


def vip_duration_text(expires_at):
    if not expires_at:
        return "♾ Butun umrga"
    dt = datetime.fromisoformat(expires_at)
    return dt.astimezone(timezone.utc).strftime("%Y-%m-%d %H:%M UTC gacha")
...
async def vip_command(message: Message, command: CommandObject):
    upsert_user(message)
    args = (command.args or "").strip().split()
    # Admin: /vip USER_ID 1 oylik | 3 oylik | 6 oylik | 8 oylik | forever
    if is_admin(message.from_user.id) and args:
        if len(args) < 2:
            await message.answer("❌ Format: <code>/vip ID 1 oylik</code> yoki <code>/vip ID forever</code>")
            return
        try:
            target = int(args[0])
        except ValueError:
            ...
            return
        duration = " ".join(args[1:]).strip().lower()
        if duration == "forever":
            grant_vip(target, "forever", message.from_user.id)
            await message.answer(f"✅ <code>{target}</code> foydalanuvchiga butun umrga VIP berildi.")
            return
        normalized = {k.lower(): k for k in VIP_PLANS}
        if duration not in normalized:
            await message.answer("❌ Muddati: 1 oylik, 3 oylik, 6 oylik, 8 oylik yoki forever.")
            return
        plan = normalized[duration]
        grant_vip(target, plan, message.from_user.id)
        await message.answer(f"✅ <code>{target}</code> foydalanuvchiga {plan} VIP berildi.")
        return

    status = await get_vip_status(message.from_user.id)
    if status:
        await message.answer(
            "💎 <b>Sizda VIP faol.</b>\n\n"
            f"⏳ {vip_duration_text(status['expires_at'])}"
        )
        return

    await message.answer(
        "💎 <b>VIP olish</b>\n\n"
        "VIP olish uchun tugmalardan birini bosing.\n"
        f"Murojaat uchun: {VIP_CONTACT}",
        reply_markup=vip_purchase_keyboard()
    )
...
async def vip_buy(callback: CallbackQuery):
    ...
    if plan not in VIP_PLANS:
        ...
        return
    prices = {"1 oylik": "15 000 so'm", "3 oylik": "39 000 so'm", "6 oylik": "69 000 so'm", "8 oylik": "90 000 so'm"}
    await callback.message.answer(
        f"💎 <b>{plan} VIP</b>\n"
        f"💰 Narxi: <b>{prices[plan]}</b>\n\n"
        f"💳 Karta raqami: <code>{VIP_CARD_NUMBER}</code>\n"
        f"🏦 Karta nomi: <b>{VIP_CARD_NAME}</b>\n"
        f"👤 Karta egasi: <b>{VIP_CARD_OWNER}</b>\n\n"
        f"📩 Murojaat uchun: {VIP_CONTACT}"
    )
...
async def vip_admin_menu(message: Message):
    if await deny_non_admin(message):
        return
    await message.answer("💎 <b>VIP boshqarish</b>", reply_markup=vip_admin_keyboard())


@router.callback_query(F.data == "vip:give")
async def vip_give_start(callback: CallbackQuery, state: FSMContext):
    if not is_admin(callback.from_user.id):
        await callback.answer("⛔ Ruxsat yo'q.", show_alert=True)
        return
    await state.set_state(VipGive.target)
    await callback.message.answer("➕ VIP berish uchun foydalanuvchi ID sini yuboring:")
    await callback.answer()


@router.message(VipGive.target)
async def vip_give_target(message: Message, state: FSMContext):
    if await deny_non_admin(message):
        return
    try:
    ...
    await state.set_state(VipGive.duration)
    await message.answer("VIP muddatini yozing: <code>1 oylik</code>, <code>3 oylik</code>, <code>6 oylik</code>, <code>8 oylik</code> yoki <code>forever</code>")


@router.message(VipGive.duration)
async def vip_give_duration(message: Message, state: FSMContext):
    if await deny_non_admin(message):
        return
    data = await state.get_data()
    duration = message.text.strip().lower()
    if duration == "forever":
        plan = "forever"
    else:
        norm = {k.lower(): k for k in VIP_PLANS}
        if duration not in norm:
            await message.answer("❌ Muddati noto'g'ri.")
            return
        plan = norm[duration]
    grant_vip(data["target"], plan, message.from_user.id)
    await state.clear()
    await message.answer(f"✅ <code>{data['target']}</code> ga VIP berildi: {plan}.", reply_markup=admin_keyboard())
...
async def vip_remove_start(callback: CallbackQuery, state: FSMContext):
    if not is_admin(callback.from_user.id):
        await callback.answer("⛔ Ruxsat yo'q.", show_alert=True)
        return
    await state.set_state(VipRemove.target)
    await callback.message.answer("➖ VIP olinadigan foydalanuvchi ID sini yuboring:")
    await callback.answer()


@router.message(VipRemove.target)
async def vip_remove_finish(message: Message, state: FSMContext):
    ...
    remove_vip(target)
    await state.clear()
    await message.answer(f"✅ <code>{target}</code> foydalanuvchidan VIP olindi.", reply_markup=admin_keyboard())
...
async def vip_list(callback: CallbackQuery):
    if not is_admin(callback.from_user.id):
        ...
        return
    clear_expired_vips()
    rows = db.execute("SELECT * FROM vip_users ORDER BY granted_at DESC").fetchall()
    if not rows:
        await callback.message.answer("📭 Hozir VIP foydalanuvchilar yo'q.")
    else:
        text = "💎 <b>VIP foydalanuvchilar</b>\n\n"
        for r in rows:
            text += f"ID: <code>{r['user_id']}</code> — {vip_duration_text(r['expires_at'])}\n"
        await callback.message.answer(text)
    await callback.answer()


@router.message(Command("vipdele"))
async def vip_delete_command(message: Message, command: CommandObject):
    if await deny_non_admin(message):
        return
    args = (command.args or "").strip().split()
    if len(args) != 1:
        await message.answer("❌ Format: <code>/vipdele ID</code>")
        return
    try:
        target = int(args[0])
    except ValueError:
        ...
        return
    remove_vip(target)
    await message.answer(f"✅ <code>{target}</code> foydalanuvchidan VIP olindi.")
...
async def code_search(message: Message):
    ...
    if message.text in {
        ...
        "📊 Statistika", "🔔 Majburiy obuna", "💎 VIP boshqarish"
    }:
        return
...
async def anime_add_genre(message: Message, state: FSMContext):
    ...
    await state.set_state(AnimeAdd.caption)
    await message.answer("📝 Kanalga joylanadigan captionni o'zingiz yozib yuboring:")


@router.message(AnimeAdd.caption)
async def anime_add_caption(message: Message, state: FSMContext):
    data = await state.get_data()
    custom_caption = message.text.strip()
    if not custom_caption:
        await message.answer("❌ Caption bo'sh bo'lmasin.")
        return
...
async def vip_expiry_worker():
    while True:
        try:
            ...
            log.exception("VIP expiry check failed")
...
async def main():
    ...
    expiry_task = asyncio.create_task(vip_expiry_worker())
    try:
